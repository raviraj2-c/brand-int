...
export default function Filter() {
  const [visible, setVisible] = useState(true); // Controls entire visibility
  ...
  const handleClearAll = () => {
    setVisible(false); // This will hide the full filter component
  };

  if (!visible) return null; // Completely hide the filter section
  ...
  return (
    <div className="p-4 bg-white rounded-md shadow-sm border border-gray-100 text-xs font-medium text-gray-800 mb-4">
      <div className="flex justify-between items-center mb-3">
        ...
        <button
          className="text-[rgba(120,60,145,1)] font-semibold text-xs"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3 items-end">
        {/* Platform, Tags, Location, Advanced Toggle */}
        <div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            className="w-full border border-gray-300 rounded-xs px-2 py-1 text-xs cursor"
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className={`w-8 h-5 flex items-center rounded-full p-0.5 transition-colors cursor-pointer duration-300 ${
              advanced ? "bg-[rgba(120,60,145,1)]" : "bg-gray-300"
            }`}
          >
            <div
              className={`w-3.5 h-3.5 bg-white rounded-full shadow-md transform duration-300 ${
                advanced ? "translate-x-3" : "translate-x-0"
              }`}
            />
          </button>
        </div>
      ...
      {/* Show sliders only if advanced is on */}
      {advanced && (
