...
export default function Signin() {
  ...
  <div className="flex max-w-3xl bg-white w-full rounded-xl overflow-hidden border border-gray-300">
    ...
    {/* Left Section */}
    <div className="left-pane w-1/2 p-6"> {/* Reduced padding */}
      <h2 className="text-3xl font-bold text-gray-900 mb-2 leading-tight"> {/* Reduced font size */}
        <span>Right </span>
      ...
      <p className="text-xs text-gray-500 mt-2">Join the best influencer finding site</p> {/* Smaller text */}
      <div className="mt-4 text-gray-700 text-xs italic"> {/* Smaller text */}
    ...
    {/* Right Section */}
    <div className="w-1/2 p-6"> {/* Reduced padding */}
      <div className="flex items-center">
        <span className="text-base font-bold tracking-wider">"</span> {/* Smaller */}
      ...
      <h2 className="p-4 text-xl font-bold text-center mb-4">Create your Account</h2> {/* Smaller text */}

      <button className="w-full flex items-center justify-center border border-gray-300 rounded-md py-2 hover:bg-gray-50 text-sm">
      ...
      <form className="space-y-2 text-sm"> {/* Smaller inputs */}
        <input placeholder="Full Name" className="input-area w-full px-3 py-2 border rounded-md" />
        <input type="email" placeholder="Email" className="input-area w-full px-3 py-2 border rounded-md" />
        <div className="relative">
          ...
          >
            {showPassword ? <Eye size={16} /> : <EyeOff size={16} />} {/* Smaller icon */}
          </button>
        </div>
        ...
        <button
          ...
          className="w-full text-white font-bold py-2 rounded-full text-sm"
        >
          SIGN UP
        </button>
      </form>
      ...
      <button
        type="button"
        onClick={() => router.push('/')}
        className="login-button font-bold w-full bg-white py-2 rounded-full border text-sm"
      >
        LOGIN
      </button>
